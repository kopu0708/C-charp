...
### 익명 메소드 
메소드는 보통 이름이 있다. 한정자가 없어도, 반환할 값이 없어도(void), 매개변수가 없어도 괜찮지만 이름만은 있어야 한다.

익명 메소드는 이름 그대로 이름이 없다는 말이다. 아니 방금 이름 있어야 한다면서요.
...
public static void Main()
{
    ...
    Calc = delegate (int a, int b) // 이름을 제외한 메소드의 구현 부분 이게 익명 메소드
            {
                return a + b;     
            };
    Console.WriteLine("3+4 : {0}", Calc(3,4)); //호출하면 위에 코드가 실행 됨 
}
```
위 예제를 보자 delegate 키워드를 이용하여 선언하여, 대리자 인스턴스가 해당 메소드를 실행하게 한다는 것을 볼 수 있다. 이게 익명 메소드이다. 

익명 메소드는 자신을 참조할 대리자의 형식과 동일한 형식으로 선언되어야 한다. 매개변수 개수도 같아야한다.

이걸 왜 쓰냐하면 사실 한번 만 쓸 메소드라면 이름을 짓기도 힘들다. 이 메소드가 한번 쓰고 다시 안 사용된다 하면은 익명 메소드를 사용할 타이밍이다.

[예제05](05_AnonymousMethod.cs)
### 이벤트: 객체에 일어난 사건 알리기 
우리는 약속을 지키기 위해 아니면 운동 루틴을 끝내거나 시작하기 위해 다양한 이유로 알람을 맞춘다. (아니면 누군가한테 알려달라고 부탁하거나)

프로그래밍에도 어떤 일이 생겼을 때 이를 알려주는 객체가 필요한 경우가 있다. 
...
```
이렇게 말이다. 프로퍼티 같은거라고 생각하면 될지도 event 붙여서 필드로 선언하고 하는 거니깐 

이게 왜 필요하냐면 event가 없으면 외부 코드가 "몬스터가 죽었다"는 사건을 조작해서 실제로 일어나지 않은 일을 일어난 것처럼 꾸밀 수 있다. 

event는 이 발생 권한을 선언된 클래스 내부로만 제한해서, 오직 진짜 그 사건이 일어났을 때만 알림이 나가도록 보장한다.

3번 즈음 간단하게 나마 만들어보니깐 손에 조금 익는 듯? 이제 람다식을 마지막으로 일단 기본기 공부는 일단락 하자 뒤에 링큐랑 스레드 에트리뷰트 같은거 궁금하긴 한데 

게임 개발이 목적이니깐 천천히 하면 됨 그리고 책만 보면 재미없어 
