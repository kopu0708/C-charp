...
## 클래스 부터 람다식 까지 마치며 
한달 걸렸는데 중간에 안놀고 했으면 훨씬 더 빨리 끝났을 듯 당연한 말이지만. 22일 기준으로 어제 즉 21일에 유니티 코리아 2026을 다녀왔었는데 정말 배운게 많았음

일단 간단하게 핸즈 온 강연? 이런 걸 신청해 뒀었어서 가서 유니티 ai, mcp에 대한 사용법을 들었음 근데 진짜 스크립트는 물론 인스펙터창을 만질 필요도 없이 2시간 만에 

게임을 뽑아내는 것을 보고 깜짝 놀랐음 물론 AI다 보니 항상 같은 결과물이 나오지는 않지만 게임의 70%% 까지는 딸각으로 뽑아 낼 수 있겠구나 싶었다.

주니어보다 못한 신생아 프로그래머로써 위기감이 드는 동시에 앞으로 훨씬더 쉽게 게임을 만들 수 있다는 기대감이 들었다.

근데 위기감이 드는 것도 사실이니 질의 응답을 하는 시간이 따로 마련되어 있어서 어떻게 해야 취업까지 할 수 있느냐 이런 질문을 했었다.

근데 3명정도 공통적으로 하는 이야기가 내가 클라이언트 개발자를 희망한다 하니 이제 그런 분류는 희미해지고 다방면으로 아는 사람이 되는게 중요하다고 말했다. 

그리고 가장 크게 깨닳은 것은 게임의 콘테츠적인 부분은 어쨌든 사람이 만들게 되어 있기에 현업에서도 AI는 게임을 만드는 툴을 만드는데 가장 많이 사용한다는 말이였다. 

즉 내 생각을 정리하자면 
1. 단순 작업(스크립트를 작성하는일 타일 맵을 작성하는 일 등)은 모두 AI의 영역으로 넘어갔다.
2. 직업군의 분류는 점점 흐려지고 있으므로 이제는 전문가가 아니라 다방면으로 구조를 아는 척척박사가 필요할 것이다.
3. 그래도 사람 손은 타기에 이제 병목현상은 코드를 해석하는 능력에 따라 해결될것이다.
4. 콘테츠 적인 부분은 사람이 만들기에 AI로 툴을 만드는데 현업은 사용중이다.
5. 열심히 하자 진짜로 
